#include "contrib/reverse_connection/bootstrap/source/reverse_connection_resolver.h"
...
ReverseConnectionResolver::resolve(const envoy::config::core::v3::SocketAddress& socket_address) {
  ...
  if (!absl::StartsWith(address_str, "rc://")) {
    return absl::InvalidArgumentError(
        fmt::format("Address must start with 'rc://' for reverse connection resolver. "
                   "Expected format: rc://src_node_id:src_cluster_id:src_tenant_id@cluster_name:count"));
  }
  ...
  auto reverse_conn_address = std::make_shared<ReverseConnectionAddress>(
      reverse_conn_config_or_error.value());

  return reverse_conn_address;
}

absl::StatusOr<ReverseConnectionAddress::ReverseConnectionConfig>
ReverseConnectionResolver::extractReverseConnectionConfig(const envoy::config::core::v3::SocketAddress& socket_address) {
  ...
  if (parts.size() != 2) {
    return absl::InvalidArgumentError("Invalid reverse connection address format. Expected: rc://src_node_id:src_cluster_id:src_tenant_id@cluster_name:count");
  }
  ...
  if (source_parts.size() != 3) {
    return absl::InvalidArgumentError("Invalid source info format. Expected: src_node_id:src_cluster_id:src_tenant_id");
  }
  ...
  if (cluster_parts.size() != 2) {
    return absl::InvalidArgumentError(fmt::format("Invalid cluster config format: {}. Expected: cluster_name:count", parts[1]));
  }

  uint32_t count;
  if (!absl::SimpleAtoi(cluster_parts[1], &count)) {
    return absl::InvalidArgumentError(fmt::format("Invalid connection count: {}", cluster_parts[1]));
  }
  ...
  ENVOY_LOG_MISC(info, "Reverse connection config: src_node_id={}, src_cluster_id={}, src_tenant_id={}, remote_cluster={}, count={}",
