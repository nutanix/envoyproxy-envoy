...
void Filter::onClose() {
  ...
  cb_->dispatcher().connectionHandler()->reverseConnRegistry().getRCManager().notifyConnectionClose(connectionKey, false /* is_used */);
...
void Filter::onPingWaitTimeout() {
  ...
  cb_->dispatcher().connectionHandler()->reverseConnRegistry().getRCManager().notifyConnectionClose(connectionKey, false);
...
Network::FilterStatus Filter::onData(Network::ListenerFilterBuffer& buffer) {
  ...
  case ReadOrParseState::Done:
    ...
    const std::string& connectionKey =
        cb_->socket().connectionInfoProvider().localAddress()->asString();
    cb_->dispatcher().connectionHandler()->reverseConnRegistry().getRCManager().markConnUsed(connectionKey);
