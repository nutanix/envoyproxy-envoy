...
ActiveReverseConnectionListener::~ActiveReverseConnectionListener() {
  ...
  for (auto& [chain, active_connections] : connections_by_context_) {
    ...
    while (!connections.empty()) {
      connections.front()->connection_->close(Network::ConnectionCloseType::NoFlush, "reverse_conn_listener_draining");
    }
  }
...
void ActiveReverseConnectionListener::startRCWorkflow(Event::Dispatcher& dispatcher,
  ...
  ENVOY_LOG(debug, "Starting reverse conn workflow on worker: {} listener: {}", dispatcher.name(),
            config.name());
  Network::LocalRevConnRegistry& local_registry = dispatcher.connectionHandler()->reverseConnRegistry();
...
void ActiveReverseConnectionListener::removeConnection(Server::ActiveTcpConnection& connection) {
  ...
  dispatcher().connectionHandler()->reverseConnRegistry().getRCManager().notifyConnectionClose(connectionKey, true /* is_used */);
...
void ActiveReverseConnectionListener::recordConnectionsAcceptedOnSocketEvent(uint32_t connections_accepted) {
