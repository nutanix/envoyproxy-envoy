...
void ReverseConnectionInitiator::reverseConnectionDone(const std::string& error,
  ...
  if (connectionClosed || error.size() > 0) {
    if (error.size() > 0) {
      std::string msg = fmt::format("Reverse connection failed: Received error {} from remote envoy",
                                    error);
      ENVOY_LOG(error, msg);
      rc_connection_manager->onFailure();
      rc_connection_manager->getConnection()->close(Network::ConnectionCloseType::NoFlush, "reverse_conn_error");
    }
