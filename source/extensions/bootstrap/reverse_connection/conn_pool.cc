...
allocateConnPool(Event::Dispatcher& dispatcher, Random::RandomGenerator& random_generator,
  ...
  ENVOY_LOG_MISC(debug,
                "Creating custom ActiveClient for reverse connections for host {}", host->getHostId());
  return std::make_unique<Http::FixedHttpConnPoolImpl>(
      host, priority, dispatcher, options, transport_socket_options, random_generator, state,
      [](Http::HttpConnPoolImplBase* pool) {
        return std::make_unique<ActiveClient>(*pool, absl::nullopt, [](Http::HttpConnPoolImplBase& pool) {
          ...
          if (cluster.type() != envoy::config::cluster::v3::Cluster_DiscoveryType::Cluster_DiscoveryType_REVERSE_CONNECTION) {
          ...
          std::pair<Network::ConnectionSocketPtr, bool> host_socket_pair =
              pool.dispatcher().connectionHandler()->reverseConnRegistry().getRCHandler().getConnectionSocket(host_id, false);
          ...
          if (!host_socket) {
            ...
            ENVOY_LOG_MISC(debug,
                           "Could not find existing socket for host {}. Creating new connection",
                           host_id);
            return host->createConnection(pool.dispatcher(), pool.socketOptions(),
                                          pool.transportSocketOptions());
          }
          ...
          Network::TransportSocketPtr&& transport_socket =
              host->transportSocketFactory().createTransportSocket(pool.transportSocketOptions(),
                                                                   host);
          ...
          Network::ClientConnectionPtr connection =
              std::make_unique<Network::ReversedClientConnectionImpl>(
                  host->address(), source_address, pool.dispatcher(), std::move(transport_socket),
                  std::move(host_socket), expects_proxy_protocol);
