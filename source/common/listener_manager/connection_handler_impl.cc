...
void ConnectionHandlerImpl::enableReverseConnections(Network::RevConnRegistry& reverse_conn_registry) {
  ...
  if (listener.has_value()) {
    ...
    for (auto address_list : listener->get().per_address_details_list_) {
      if (address_list->reverseConnectionListener().has_value()) {
        ENVOY_LOG(debug, "passing reverse connection to listener {} tag {}",
                  listener->get().name_, listener_tag);
        address_list->reverseConnectionListener()->onAccept(std::move(upstream_socket));
        ENVOY_LOG(debug, "reverse connection passed to listener {} tag {}",
                  listener->get().name_, listener_tag);
        return;
      } else {
        ENVOY_LOG(debug, "no reverse connection listener for name {} tag {}", listener->get().name_, listener_tag);
      }
    }
  }
}
...
ConnectionHandlerImpl::PerAddressActiveListenerDetails::reverseConnectionListener() {
  ...
  auto* val = absl::get_if<
      ...
      &typed_listener_);
  if (val == nullptr) {
    ENVOY_LOG(debug, "reverseConnectionListener is nullptr");
  }
  return (val != nullptr) ? makeOptRef(val->get()) : absl::nullopt;
}
