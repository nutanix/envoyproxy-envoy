...
using CreateConnectionDataFn = std::function<Upstream::Host::CreateConnectionData(HttpConnPoolImplBase& pool)>;
...
public:
  ActiveClient(HttpConnPoolImplBase& parent, uint64_t lifetime_stream_limit,
               ...
               CreateConnectionDataFn connection_fn = [](HttpConnPoolImplBase& parent) {
                 return static_cast<Envoy::ConnectionPool::ConnPoolImplBase*>(&parent)->host()->createConnection(parent.dispatcher(), parent.socketOptions(), parent.transportSocketOptions());
               })
