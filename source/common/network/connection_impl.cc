...
void ConnectionImpl::closeConnectionImmediately() {
  closeSocket(ConnectionEvent::LocalClose); 
}
...
ClientConnectionImpl::ClientConnectionImpl(Event::Dispatcher& dispatcher,
    ...
    : ConnectionImpl(dispatcher, std::move(downstream_socket), std::move(transport_socket),
                     stream_info_, false),
      stream_info_(dispatcher.timeSource(), socket_->connectionInfoProviderSharedPtr(), StreamInfo::FilterState::LifeSpan::Connection) {
