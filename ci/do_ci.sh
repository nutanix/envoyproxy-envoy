...
function bazel_with_collection() {
  ...
  # File "/source/ci/flaky_test/process_xml.py", line 6, in <module>
  #   import slack
...
if [[ "$CI_TARGET" == "bazel.release" ]]; then
  ...
  echo "Testing //test/exe:envoy_static_test"
  bazel_with_collection test ${BAZEL_BUILD_OPTIONS[@]} -c opt "//test/exe:envoy_static_test"
  BAZEL_BUILD_OPTIONS+=("--define" "dynamic_link_tests=true")
  ...
  setup_clang_toolchain
  BAZEL_BUILD_OPTIONS+=(-c dbg --define dynamic_link_tests=true "--config=clang-asan" "--build_tests_only" "--remote_download_minimal")
  ...
  echo "Building and testing envoy tests ${TEST_TARGETS[*]}"
  bazel_with_collection test --config=rbe-toolchain-tsan --test_timeout=1000 --define dynamic_link_tests=true "${BAZEL_BUILD_OPTIONS[@]}" -c dbg --build_tests_only --remote_download_minimal "${TEST_TARGETS[@]}"
  if [ "${ENVOY_BUILD_FILTER_EXAMPLE}" == "1" ]; then
    echo "Building and testing envoy-filter-example tests..."
    pushd "${ENVOY_FILTER_EXAMPLE_SRCDIR}"
  ...
  # Nutanix specific: Add the flag dynamic_link_tests to save disk space by linking shared libraries.
  ...
  echo "Testing API..."
  # Nutanix specific: We do not want to test the tap2pcap_test as it is not relevant for us.
...
#    @envoy_api//tools:tap2pcap_test
  ...
  exit 0
    ...
    exit 0
elif [[ "$CI_TARGET" == "bazel.go_protos.api" ]]; then
